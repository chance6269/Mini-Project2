...
def make_charts(loc):
    ...
    file = '{}_월별_실거래지수_이동데이터.xlsx'.format(loc)
    df = pd.read_excel(folder_path+file, index_col=0)
    
    corr = df.corr()
    
    corr_order = df.corr().loc['총전입 (명)':,'실거래지수'].abs().sort_values(ascending=False)
    corr_order
    ...
    # 월실거래지수 분포
    sns.displot(x='실거래지수',kind='hist', data=df)
    plt.savefig(img_path + '{}_월실거래지수_분포도.png'.format(loc))
    plt.show()
    ...
    ax.plot(df.index, df['실거래지수'], label='월실거래지수')
    ax.plot(df.index, df['순이동 (명)'],label='순이동')
    
    ax.legend()
    ax.set_title('{} 월실거래지수/순이동 추이'.format(loc))
    plt.xticks(range(0,len(df),3),df.index[::3],rotation=45)
    fig.savefig(img_path + '{}_월실거래지수-순이동_추이.png'.format(loc))
    fig.show()
    ...
    ax.legend()
    ax.set_title('{} 월실거래지수/전입 추이'.format(loc))
    plt.xticks(range(0,len(df),3),df.index[::3],rotation=45)
    fig.savefig(img_path + '{}_월실거래지수-전입_추이.png'.format(loc))
    fig.show()
    ...
    ax.legend()
    ax.set_title('{} 월실거래지수/전출 추이'.format(loc))
    plt.xticks(range(0,len(df),3),df.index[::3],rotation=45)
    fig.savefig(img_path + '{}_월실거래지수-전출_추이.png'.format(loc))
    fig.show()
    ...
    ax.legend()
    ax.set_title('{} 월실거래지수/시도내이동 추이'.format(loc))
    plt.xticks(range(0,len(df),3),df.index[::3],rotation=45)
    fig.savefig(img_path + '{}_월실거래지수-시도내이동_추이.png'.format(loc))
    fig.show()